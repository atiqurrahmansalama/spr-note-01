...
export default function SubjectRoutineStudioView({
  ...
  const [activeLens, setActiveLens] = useState(LENS_MODES.ALL);
  const [density, setDensity] = useState('normal'); // 'compact' | 'normal' | 'comfortable'

  // ─── Pre-fill state for clicking an empty cell ──────────────────────────────
  const [targetSlotPrefill, setTargetSlotPrefill] = useState(null);
  ...
  const matrixColumns = useMemo(() => {
    ...
    const addColumn = (dateStr, shiftObj, dayMeta = null) => {
      if (!dateStr) return;
    ...
    if (colMap.size === 0 && Array.isArray(designatedExamDays) && designatedExamDays.length > 0) {
      designatedExamDays.forEach((dStr) => {
    ...
    const sorted = Array.from(colMap.values()).sort((a, b) => {
  ...
  const matrixRows = useMemo(() => {
    ...
    const normalize = (s) => String(s || '').replace(/^dept_|^cls_/, '').trim().toLowerCase();

    const addClass = (clsObj) => {
    ...
    return Array.from(classMap.values())
      ...
      .map((c) => {
        ...
        const matchedDept = (departments || []).find((d) => String(d.id) === deptId);
  ...
  }, [participatingClasses, allAvailableClasses, routineRows, filterDepartmentId, filterClassId, departments]);

  // ─── 4. Bulletproof Cell Item Matching Logic ──────────────────────────────
  const cellItemExtractor = useCallback((allData, row, col) => {
  ...
  const handleItemMove = useCallback(
    (draggedItem, sourceRow, sourceCol, targetRow, targetCol, scope = LENS_MODES.ALL) => {
  ...
  const handleItemSwap = useCallback(
    (draggedItem, targetItem, sourceRow, sourceCol, targetRow, targetCol, scope = LENS_MODES.ALL) => {
  ...
  useDrawerRegistration(
    ...
    (params) => {
      ...
      const foundRow =
        mode === 'edit' && rowId
          ? routineRows.find((r) => String(r.id) === String(rowId)) ||
  ...
  return (
    <div className="space-y-4 animate-fade-in text-left">
      ...
      <TimetableMatrixGrid
        ...
        onCellDelete={(item) => setRowToDelete(item)}
