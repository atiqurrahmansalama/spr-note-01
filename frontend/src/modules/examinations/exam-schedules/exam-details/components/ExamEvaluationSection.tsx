...
import { ExamEvaluationSectionProps, RankingScope, FailSubjectRule } from '../types';
        ...
        {breakdownEnabled && (
          <div className="space-y-3.5 p-3.5 rounded-xl border theme-border theme-bg-sub/30 shadow-2xs animate-fade-in">
            ...
            <div className="flex items-center justify-between text-xs pt-2 border-t theme-border">
              ...
              >
                {componentsTotalMarks} / {targetFullMarks} pts{' '}
                {componentsTotalMarks !== Number(targetFullMarks) && '(Must equal Full Marks)'}
              </span>
            </div>
          </div>
        )}
          ...
          <div className="space-y-3.5 p-3.5 rounded-xl border theme-border theme-bg-sub/30 shadow-2xs animate-fade-in">
            {previousExams.length === 0 ? (
              <EmptyState
                icon={HistoryIcon}
                iconVariant="default"
                title="No previous exam terms linked yet"
                description="Link a prior examination (e.g. 1st Term, Mid-Term) to merge marks into the final result."
                variant="dashed"
                size="sm"
                action={{
                  label: 'Link a Previous Exam',
                  icon: PlusIcon,
                  onClick: onAddPreviousExam,
                  variant: 'primary',
                  size: 'xs',
      ...
      >
        <div className="grid grid-cols-1 @[480px]:grid-cols-2 gap-3.5">
          <CustomSelect
            ...
            value={rankingScope}
            onChange={(val) => onRankingScopeChange(val as RankingScope)}
          />
          ...
          <CustomSelect
            ...
            onChange={(val) => onFailSubjectRuleChange(val as FailSubjectRule)}
          />
        </div>
      </DrawerSection>
    </div>
