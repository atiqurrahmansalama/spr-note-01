...
export default function useExamFormState({
  ...
  const [isDraftRestored, setIsDraftRestored] = useState<boolean>(() => Boolean(savedDraft));
  const [saving, setSaving] = useState<boolean>(false);
  const [autoSaveStatus, setAutoSaveStatus] = useState<string>('saved');
  const [lastSavedAt, setLastSavedAt] = useState<string | null>(() =>
    savedDraft?.updatedAt
      ? new Date(savedDraft.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      : null
  );

  // ── Step 1: General Info & Schedule ───────────────────────────────────────
  const [name, setName] = useState<LocalizedFieldInput>(savedDraft?.name ?? exam?.name ?? '');
  const [code, setCode] = useState<string>(savedDraft?.code ?? exam?.code ?? '');
  ...
  const [academicYearId, setAcademicYearId] = useState<string>(() => {
    ...
    const activeY = academicYears.find((y: any) => y.isCurrent || y.is_active);
    return activeY ? String(activeY.id || activeY.academic_year) : (academicYearOptions[0]?.value || '');
  });

  const selectedYear = useMemo(() => {
    if (!academicYearId) {
      return academicYears.find((y: any) => y.isCurrent || y.is_active) || academicYears[0] || null;
    }
    const found = academicYears.find(
      (y: any) => String(y.id || y.academic_year || y.year_code) === String(academicYearId)
    );
    return found || academicYears.find((y: any) => y.isCurrent || y.is_active) || academicYears[0] || null;
  ...
  const semesterOptions = useMemo<SelectOption[]>(() => {
    ...
    return selectedYear.terms.map((t: any, idx: number) => {
  ...
  const [semesterId, setSemesterId] = useState<string>(() => {
    ...
    const currentTerm = selectedYear?.terms?.find((t: any) => t.isCurrent);
    return currentTerm ? String(currentTerm.id) : (semesterOptions[0]?.value || 'annual_term');
  });

  const [gradingSystemId, setGradingSystemId] = useState<string>(
    savedDraft?.gradingSystemId ?? exam?.gradingSystemId ?? gradingSystemOptions[0]?.value ?? 'dars_e_nizami_standard'
  );

  const [startDate, setStartDate] = useState<string>(
    savedDraft?.startDate ?? exam?.startDate ?? new Date().toISOString().split('T')[0]
  );
  const [endDate, setEndDate] = useState<string>(
    savedDraft?.endDate ?? exam?.endDate ?? new Date().toISOString().split('T')[0]
  );
  const [prepStartDate, setPrepStartDate] = useState<string>(
    savedDraft?.prepStartDate ?? exam?.prepStartDate ?? ''
  );
  const [prepEndDate, setPrepEndDate] = useState<string>(
    savedDraft?.prepEndDate ?? exam?.prepEndDate ?? ''
  );
  const [scheduleDays, setScheduleDays] = useState<ExamScheduleDay[]>(
    savedDraft?.scheduleDays ?? exam?.scheduleDays ?? []
  );
  const [description, setDescription] = useState<string>(
    savedDraft?.description ?? exam?.description ?? ''
  );
  ...
  const [shifts, setShifts] = useState<ExamShift[]>(() => {
    ...
    if (Array.isArray(exam?.shifts) && exam.shifts.length > 0) {
      return exam.shifts.map((s: any, idx: number) => ({
  ...
  const handleShiftChange = (index: number, field: keyof ExamShift, value: string) => {
    setShifts((prev) =>
  ...
  const [departmentId, setDepartmentId] = useState<string | string[]>(() => {
    if (savedDraft?.departmentId !== undefined || savedDraft?.departmentIds !== undefined) {
      return normalizeDepartmentIdList(savedDraft.departmentIds ?? savedDraft.departmentId);
    }
    if (exam?.departmentId !== undefined || exam?.departmentIds !== undefined) {
      return normalizeDepartmentIdList(exam.departmentIds ?? exam.departmentId);
    }
    return ['ALL'];
  });

  const [isMultiDepartmentSchedule, setIsMultiDepartmentSchedule] = useState<boolean>(
  ...
  const activeDepartments = useMemo(() => {
    const deptsMap = new Map<string, { label: string; code: string }>();
    const cleanDepts = (departmentOptions || []).filter((d) => d.value && d.value !== 'ALL');
    const currentDeptList = normalizeDepartmentIdList(departmentId);
  ...
  const handleUpdateDepartmentSchedule = (
    deptId: string,
    ...
    setDepartmentSchedules((prev) =>
      prev.map((ds) => {
        if (String(ds.departmentId) === String(deptId)) {
          ...
          return { ...ds, [fieldOrObject as keyof DepartmentSchedule]: value };
        }
        return ds;
      })
    );
  ...
  useEffect(() => {
    if (exam?.departmentId !== undefined && exam?.departmentId !== null && !savedDraft) {
      setDepartmentId(normalizeDepartmentIdList(exam.departmentId));
    }
  ...
  const handleDepartmentChange = (newDeptVal: string | string[]) => {
    const nextList = normalizeDepartmentIdList(newDeptVal);
  ...
  const handleClassToggle = (cId: string | number) => {
  ...
  const [breakdownEnabled, setBreakdownEnabled] = useState<boolean>(
    savedDraft?.breakdownEnabled ?? exam?.breakdownEnabled ?? true
  );
  const [targetFullMarks, setTargetFullMarks] = useState<number | string>(
    savedDraft?.targetFullMarks ?? exam?.defaultFullMarks ?? 100
  );

  const [defaultComponents, setDefaultComponents] = useState<ExamAssessmentComponent[]>(() =>
    getDefaultComponents(exam, savedDraft)
  );
  ...
  const handleUpdateComponent = (
    index: number,
    field: keyof ExamAssessmentComponent,
    value: any
  ) => {
    setDefaultComponents((prev) =>
      prev.map((c, idx) => {
  ...
  const handleTargetFullMarksChange = (val: string | number) => {
    const num = Math.max(1, parseInt(String(val), 10) || 0);
    setTargetFullMarks(num);
  };

  // CA & Mark Distribution Weightage
  const [caEnabled, setCaEnabled] = useState<boolean>(
    savedDraft?.caEnabled ?? exam?.caWeightage?.enabled ?? false
  );
  const [dailyEnabled, setDailyEnabled] = useState<boolean>(
    savedDraft?.dailyEnabled ?? exam?.caWeightage?.dailyEnabled ?? true
  );
  const [attendanceEnabled, setAttendanceEnabled] = useState<boolean>(
    savedDraft?.attendanceEnabled ?? exam?.caWeightage?.attendanceEnabled ?? true
  );
  const [examWeightageEnabled, setExamWeightageEnabled] = useState<boolean>(
    savedDraft?.examWeightageEnabled ?? exam?.caWeightage?.examWeightageEnabled ?? true
  );

  const [dailyClassroomPct, setDailyClassroomPct] = useState<number>(
    savedDraft?.dailyClassroomPct ?? exam?.caWeightage?.dailyClassroomPct ?? 10
  );
  const [attendancePct, setAttendancePct] = useState<number>(
    savedDraft?.attendancePct ?? exam?.caWeightage?.attendancePct ?? 10
  );
  const [examPct, setExamPct] = useState<number>(
    savedDraft?.examPct ?? exam?.caWeightage?.examPct ?? 80
  );

  // Previous Exams Marks Merger
  const allExams = useMemo<Exam[]>(() => {
    return examStore.getExams(tenantId);
  }, [tenantId]);

  const otherExamsOptions = useMemo<SelectOption[]>(() => {
    return allExams
      .filter(
        (e) =>
          String(e.id) !== String(exam?.id) &&
          (!academicYearId || String(e.academicYearId) === String(academicYearId))
      )
      .map((e) => {
        const examTitle =
          typeof e.name === 'string'
            ? e.name
            : getLocalizedValue(e.name, 'en') || 'Exam';
        return {
          value: String(e.id),
          label: `${examTitle} (${e.semesterName || 'Term'} — Full Marks: ${e.defaultFullMarks || e.targetFullMarks || 100} pts)`,
          exam: e,
        };
      });
  }, [allExams, exam?.id, academicYearId]);

  const [previousExamsEnabled, setPreviousExamsEnabled] = useState<boolean>(
    savedDraft?.previousExamsEnabled ?? exam?.previousExamsConfig?.enabled ?? false
  );
  const [previousExams, setPreviousExams] = useState<PreviousExamItem[]>(() => {
  ...
  const handleAddPreviousExam = () => {
    setPreviousExams((prev) => {
      const nextIdx = prev.length + 1;
      const defaultExamId =
        otherExamsOptions.find((opt) => !prev.some((p) => p.examId === opt.value))?.value ||
        otherExamsOptions[0]?.value ||
        '';
      return [
        ...prev,
        {
  ...
  const handleRemovePreviousExam = (indexToRemove: number) => {
    setPreviousExams((prev) => prev.filter((_, idx) => idx !== indexToRemove));
  };

  const handleUpdatePreviousExam = (
    index: number,
    field: keyof PreviousExamItem,
    value: any
  ) => {
    setPreviousExams((prev) =>
      prev.map((item, idx) => {
  ...
  const handleCaChange = (field: 'daily' | 'attendance' | 'exam', val: string | number) => {
    const num = Math.max(0, Math.min(100, parseInt(String(val), 10) || 0));
  ...
  // Ranking & Merit Rules
  const [rankingScope, setRankingScope] = useState<RankingScope>(
    savedDraft?.rankingScope ?? exam?.rankingConfig?.scope ?? 'CLASS_AND_SECTION'
  );
  const [failSubjectRule, setFailSubjectRule] = useState<FailSubjectRule>(
    savedDraft?.failSubjectRule ?? exam?.rankingConfig?.failSubjectRule ?? 'EXCLUDE_FROM_MERIT'
  );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (isSavedRef.current) return;
      const hasName = typeof name === 'string' ? Boolean(name.trim()) : hasAnyLanguageContent(name);
  ...
  const handleDiscardDraft = () => {
    ...
    setDepartmentId(normalizeDepartmentIdList(exam?.departmentIds ?? exam?.departmentId));
    ...
    setShifts(
      Array.isArray(exam?.shifts) && exam.shifts.length > 0
        ? exam.shifts.map((s: any, idx: number) => ({
            id: s.id || `shift_${idx + 1}`,
            name: s.name || `Shift ${idx + 1}`,
            startTime: s.startTime || '09:00 AM',
            endTime: s.endTime || '11:00 AM',
    ...
    setDefaultComponents(getDefaultComponents(exam));
    ...
    setRankingScope((exam?.rankingConfig?.scope as RankingScope) || 'CLASS_AND_SECTION');
    setFailSubjectRule((exam?.rankingConfig?.failSubjectRule as FailSubjectRule) || 'EXCLUDE_FROM_MERIT');
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    try {
      const selectedYearObj =
        academicYears.find((y: any) => String(y.id || y.academic_year || y.year_code) === String(academicYearId)) ||
        selectedYear;
      ...
      const payload: Partial<Exam> = {
        name: typeof name === 'string' ? name.trim() : (name as LocalizedValue),
        ...
        defaultBreakdown: {
          written:
            Number(
              (defaultComponents.find((c) => c.name.toLowerCase().includes('written')) || defaultComponents[0])
                ?.maxMarks
            ) || 70,
          oral:
            Number(
              (
                ...
                ) || (defaultComponents.length > 1 ? defaultComponents[1] : { maxMarks: 0 })
              )?.maxMarks
            ) || 30,
        },
      ...
      try {
        ...
        const existingIdx = calendarEvents.findIndex(
          (e: any) => e.id === examEventId || (e.examId && String(e.examId) === String(savedId))
        );
        ...
        const calendarTitle =
          typeof payload.name === 'string'
            ? payload.name
            : getLocalizedValue(payload.name, 'en') || 'Examination';

        const calendarPayload: any = {
          id: examEventId,
          examId: savedId,
          title: `${calendarTitle} (Examination)`,
          category: 'EXAM',
          audience: 'STUDENTS',
        ...
        if (existingIdx >= 0) {
          ...
          masterCalendarStore.saveEvents(tenantId, calendarEvents as any);
        } else {
          masterCalendarStore.addEvent(tenantId, calendarPayload as any);
        }
      } catch (syncErr) {
