...
export interface SubjectScheduleTimingSectionProps {
  formData: any;
  setFormData: React.Dispatch<React.SetStateAction<any>>;
  departmentSchedule?: any;
  dateOptions?: SelectOption[];
  shiftOptions?: SelectOption[];
  handleShiftChange: (shiftId: string) => void;
}
...
export default function SubjectScheduleTimingSection({
  ...
  shiftOptions = [],
  handleShiftChange,
}: SubjectScheduleTimingSectionProps) {
  return (
        ...
        <div className="grid grid-cols-1 @[480px]:grid-cols-2 gap-3.5">
          {dateOptions.length > 0 ? (
            <CustomSelect
              ...
              options={dateOptions}
              icon={CalendarIcon}
              onChange={(val: string) => setFormData((prev: any) => ({ ...prev, examDate: val }))}
              required
            />
          ) : (
            <CustomInput
          ...
          {shiftOptions.length > 0 ? (
            ...
            <CustomInput
              ...
              onChange={(val: string) => setFormData((prev: any) => ({ ...prev, shiftName: val }))}
              icon={ClockIcon}
            />
          )}
        </div>
