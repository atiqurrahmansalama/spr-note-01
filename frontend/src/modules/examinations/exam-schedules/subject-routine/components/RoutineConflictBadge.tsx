...
export interface RoutineConflictBadgeProps {
  conflictInfo?: {
    hasConflict?: boolean;
    severity?: 'danger' | 'warning' | string;
    conflicts?: Array<{ title?: string; message?: string; [key: string]: any }>;
 ...
 */
export default function RoutineConflictBadge({ conflictInfo }: RoutineConflictBadgeProps) {
    ...
    >
      <div
        className={`flex items-center justify-center transition-transform hover:scale-125 cursor-help ${
          isDanger ? 'theme-danger animate-pulse' : 'theme-warning'
        }`}
