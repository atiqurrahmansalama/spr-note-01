...
import { SubjectRoutineDrawerFormProps, SubjectRoutineItem, RoutineAssessmentComponent, RoutinePreviousExamMark } from '../types';
...
export default function useSubjectRoutineForm({
  ...
  const defaultPreviousExamsList: RoutinePreviousExamMark[] = useMemo(() => {
  ...
  const [formData, setFormData] = useState<SubjectRoutineItem>(() => {
    ...
    if (initialData) {
      ...
      return {
        id: String(initialData.id || `subj_${Date.now()}`),
        examId: String(initialData.examId || activeExam?.id || ''),
    ...
    return {
      ...
      examId: activeExam?.id ? String(activeExam.id) : '',
  ...
  const classMatchingBooks = useMemo(() => {
    ...
    return availableCurriculumBooks.filter((b: any) => {
  ...
  const departmentSchedule = useMemo(() => {
    ...
    return (
      activeExam.departmentSchedules.find(
        (ds: any) => String(ds.departmentId).trim() === currentDeptId
      ) || null
    );
  ...
  const handleDepartmentChange = (deptId: string, deptObj?: any) => {
  ...
  const handleClassChange = (selectedVal: string, selectedClassObj?: any) => {
  ...
  const handleSectionChange = (selectedVal: string, selectedSectionObj?: any) => {
  ...
  const handleBookChange = (selectedBookId: string, opt?: any) => {
    ...
    const matchedBook = opt?.bookObj || availableCurriculumBooks.find((b: any) => String(b.id) === String(selectedBookId));
    if (matchedBook) {
  ...
  const handleShiftChange = (shiftId: string, opt?: any) => {
    const shift = opt?.shiftObj || examShifts.find((s: any) => String(s.id) === String(shiftId));
    if (shift) {
      setFormData((prev) => ({
        ...prev,
        shiftId: shift.id,
  ...
  const handleTeacherChange = (selectedVal: string, teacherObj?: any) => {
  ...
  const handleToggleBreakdown = (checked: boolean) => {
  ...
  const handleAddComponent = () => {
    ...
    const newComp: RoutineAssessmentComponent = {
  ...
  const handleRemoveComponent = (idxToRemove: number) => {
    setFormData((prev) => {
  ...
  const handleTogglePreviousExams = (checked: boolean) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const handleUpdatePreviousExamWeight = (idx: number, val: number | string) => {
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    const matchedClass = allAvailableClasses.find((c: any) => String(c.id) === String(formData.classId));
    ...
    const matchedBook = formData.curriculumBookId
      ? availableCurriculumBooks.find((b: any) => String(b.id) === String(formData.curriculumBookId))
      : null;
    const resolvedBookName = formData.curriculumBookName || matchedBook?.name || matchedBook?.title || '';

    const matchedShift = examShifts.find((s: any) => String(s.id) === String(formData.shiftId)) || examShifts[0];
    ...
    const payload: SubjectRoutineItem = {
      ...formData,
