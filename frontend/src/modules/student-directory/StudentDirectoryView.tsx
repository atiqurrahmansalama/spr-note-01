...
export default function StudentDirectoryView({ viewMode = "all" }: StudentDirectoryViewProps) {
  ...
  // Modals
  const [bulkActionType, setBulkActionType] = useState<string>("change_status");
  const [bulkGroupInput, setBulkGroupInput] = useState<string>("");
  ...
  const getActionMenuItems = (s: StudentRecord) => [
    {
      label: "View Profile",
      icon: SearchIcon,
  ...
  const renderStudentCard = (s: StudentRecord) => (
    <div
      key={s.id}
      onClick={() => navigate(`/students/${s.id}/profile`)}
        ...
        <div className="p-5 sm:p-6 space-y-4 text-left">
          <div>
            <CustomSelect
              ...
              onChange={(val: string) => setBulkActionType(val)}
              options={[
          ...
          {bulkActionType === "change_status" && (
            <div>
              <CustomSelect
                ...
                onChange={(val: string) => setBulkStatusInput(val)}
                options={[
