...
export default function BedAllocationModal({
  ...
  const [selectedStaffObj, setSelectedStaffObj] = useState<any>(null);
    ...
    try {
      ...
      if (allocType === 'STUDENT') {
        ...
        if (!found && !studentSearch.trim()) {
          showToast('Please select or specify a student', 'warning');
          setSaving(false);
          return;
        }
        studentData = found || {
        ...
        staffData = {
          id: selectedStaffId,
          name: selectedStaffObj?.name || selectedStaffObj?.label || selectedStaffObj?.username || 'Staff Member',
        };
      }
