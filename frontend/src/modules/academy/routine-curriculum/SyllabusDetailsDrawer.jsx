        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1.5 text-xs theme-text-secondary flex-wrap">
            <span className="flex items-center gap-1.5">
              ...
              <strong className="theme-text-primary font-semibold">{item.className || 'All Classes'}</strong>
            </span>
            {item.periodName && (
              <>
                <span>•</span>
            ...
            <div className="flex items-center justify-between py-1.5 border-b theme-border">
              ...
              <span className="font-semibold theme-text-primary">
                ...
                <span className="theme-accent font-medium">
                  ({item.scheduleType === 'SPLIT_DAYS' && item.scheduleDays?.length > 0
                    ? `Split: ${item.scheduleDays.map((d) => d.slice(0, 3)).join(', ')}`
                    : 'Full Week'})
                </span>
              </span>
            </div>
      ...
      <div className="pt-3 border-t theme-border flex items-center justify-between gap-2.5 mt-auto flex-wrap">
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3.5 py-2 rounded-xl theme-bg-accent theme-accent-text text-xs font-bold hover:opacity-90 shadow-sm flex items-center gap-1.5 cursor-pointer transition"
          >
          ...
          <button
            type="button"
            onClick={() => onEdit(item)}
            className="px-3.5 py-2 rounded-xl border theme-border text-xs font-bold theme-text-primary hover:theme-bg-sub shadow-2xs flex items-center gap-1.5 cursor-pointer transition"
          >
        ...
        <button
          ...
          className="px-3 py-2 rounded-xl border theme-border text-xs font-bold text-[var(--danger-main)] hover:bg-[var(--danger-soft)] shadow-2xs flex items-center gap-1.5 cursor-pointer transition ml-auto"
        >
