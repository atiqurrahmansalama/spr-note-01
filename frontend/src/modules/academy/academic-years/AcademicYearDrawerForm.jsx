...
export default function AcademicYearDrawerForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col h-full space-y-5 pb-2">
        ...
        <div className="flex items-center justify-between pb-1">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-xs font-bold uppercase tracking-wider theme-text-primary">
              Terms & Semesters ({formData.terms.length})
            </h4>
          </div>
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-xl text-xs font-bold theme-bg-accent-soft theme-accent hover:brightness-95 transition flex items-center gap-1 cursor-pointer border border-[var(--accent-main)]/20 shadow-2xs"
          >
          ...
          {formData.terms.map((term, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  {formData.terms.length > 1 && (
                    <button
                      ...
                      className="p-1.5 rounded-lg text-zinc-400 hover:text-rose-500 hover:theme-bg-sub transition cursor-pointer"
                      title="Remove Term"
                    >
                ...
                {/* Line 2: Start Date & End Date (Guarded 2-Column Grid) */}
      ...
      {/* ─── 4. Action Buttons ────────────────────────────────────── */}
      <div className="flex items-center justify-end gap-2.5 pt-4 border-t theme-border mt-auto">
        <button
          type="button"
          onClick={onCancel}
          className="px-4 py-2 rounded-xl text-xs font-semibold border theme-border theme-text-secondary hover:theme-text-primary hover:theme-bg-sub transition cursor-pointer"
        >
          Cancel
        </button>
        <button
          type="submit"
          className="px-5 py-2 rounded-xl text-xs font-bold theme-bg-accent theme-accent-text hover:opacity-90 shadow-md transition-all flex items-center gap-1.5 cursor-pointer"
        >
          <SaveIcon className="w-3.5 h-3.5" />
          <span>{year ? "Update Academic Year" : "Save Academic Year"}</span>
        </button>
      </div>
    </form>
  );
