...
export default function SectionMigrationModal({
  ...
  if (!isOpen || !deletingSection) return null;

  // Filter out self to ensure Self-Target Prevention
  const eligibleSections = availableSections.filter(
    (s) => s.id !== deletingSection.id && !s.is_deleted
  );

  const handleMigrateAndDelete = async () => {
    if (!targetSectionId) {
      showToast('Please select a destination section for migration.', 'warning');
      return;
    }

    if (targetSectionId === deletingSection.id) {
      showToast('Destination section cannot be the same section (Self-Migration Prohibited).', 'error');
      return;
    }
    ...
    try {
      const res = await fetchWithAuth(
        `/api/v1/academy/sections/${deletingSection.id}/delete-with-migration/`,
        {
          method: 'POST',
      ...
      if (res.ok) {
        ...
        showToast(
          data.message || `Successfully migrated students to ${data.target_section}!`,
          'success'
        );
        onSuccess?.();
        onClose();
        ...
        showToast(err.error || 'Failed to execute section migration.', 'error');
      }
    } catch {
      showToast('Network error during section migration.', 'error');
    } finally {
      setMigrating(false);
    }
  };
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold theme-text-primary">
                Migrate Students &amp; Delete Section
              </h3>
              <p className="text-xs theme-text-secondary mt-0.5">
                Section <span className="font-semibold theme-text-primary">"{deletingSection.section_name || deletingSection.name}"</span> has enrolled students.
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-4 rounded-xl theme-bg-amber-soft/50 border theme-border-amber space-y-2 text-xs">
          ...
          <p className="theme-text-secondary leading-relaxed">
            All active students currently assigned to <strong className="theme-text-primary">{deletingSection.section_name || deletingSection.name}</strong> will be safely migrated to your selected destination section before this section is permanently deleted.
          </p>
        </div>

        {/* Target Destination Section Selector */}
        <div className="space-y-2">
          <label className="text-xs font-bold theme-text-primary block">
            Target Destination Section <span className="text-rose-500">*</span>
          </label>
          <CustomSelect
            ...
            options={eligibleSections.map((s) => ({
              value: String(s.id),
              label: `${s.section_name || s.name} — ${s.student_class_name || 'Class'} (${s.branch_name || 'All Branches'})`,
            }))}
          ...
          {eligibleSections.length === 0 && (
            <p className="text-[11px] text-rose-500 mt-1">
              No alternative sections available in this institution. Please create another section first.
            </p>
          )}
        </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t theme-border">
          ...
          <button
            ...
            disabled={migrating || !targetSectionId || eligibleSections.length === 0}
