...
import { getEventColors } from '@/components/calendar/MasterTimeCalendar';
import type {
  UseAttendanceDateManagerParams,
  UseAttendanceDateManagerReturn,
  DayPeriodItem,
} from '../types';
 ...
 */
export function isEventImpactedForModule(evt: any, moduleType = 'ALL'): boolean {
  if (!evt) return false;
  ...
  const normImpacts = rawImpacts.map((i: any) => String(i).toUpperCase().replace(/[^A-Z0-9_]/g, '_'));
...
export function useAttendanceDateManager({
  ...
  initialStartDate = '',
  initialEndDate = '',
}: UseAttendanceDateManagerParams = {}): UseAttendanceDateManagerReturn {
  ...
  const [selectedMonth, setSelectedMonth] = useState<number>(resolvedInitialMonth);
