...
export default function LessonDeliveryManagementView({
  ...
  const lessonColumns = [
    ...
    },
    {
      ...
      render: (row) => (
        <span
          className="text-xs theme-text-secondary line-clamp-2 max-w-[240px] block"
  ...
  const renderLessonCard = (row) => {
    ...
    return (
      <div
        key={row.id}
        className="rounded-2xl border theme-border theme-bg-primary p-4 sm:p-5 flex flex-col justify-between space-y-3.5 hover:shadow-md transition-shadow text-left"
      >
