...
export const PdfReportPreview = forwardRef<HTMLDivElement, PdfReportPreviewProps>(function PdfReportPreview(
  { reportData, includeGroup = true, includeTeacher = true },
  ref
) {
  ...
  const countValidItems = (data?: DetailRowData[]) => {
    return (data || []).reduce((total, row) => {
      if (!row.page || String(row.page).trim() === "") return total;
      const validAyahs = (row.ayahs || []).filter((a) => a.value && String(a.value).trim() !== "").length;
  ...
  const getDetailRows = (data?: DetailRowData[]) => {
    const detailByJuz = new Map<string, string[]>();

    (data || []).forEach((row) => {
      if (!row.page || String(row.page).trim() === "") return;
      const juzNum = (row.juz || validJuzs[0] || "").toString().trim();
      const pageStr = String(row.page).trim();
  ...
  return (
    <div
      ref={ref}
      className="w-full bg-white text-slate-800 rounded-xl shadow-md border border-slate-200 overflow-hidden max-w-lg mx-auto font-[system-ui,sans-serif] select-all text-left"
    >
      {/* Document Top Bar */}
      <div className="bg-slate-800 px-5 py-3 flex items-center justify-between">
        <span className="text-white text-[11px] font-bold uppercase tracking-widest opacity-80">
      ...
      <div className="p-5 space-y-3">
        ...
        <div className="space-y-1 pb-3 border-b border-slate-100">
          {showHeaderDate && (
            <div className="flex items-baseline gap-2 text-[14px]">
              <span className="font-semibold text-slate-500 w-28 shrink-0 text-[12px] uppercase tracking-wide">
                Date
              </span>
                ...
                {isSingleJuz ? (
                  <div className="flex items-baseline gap-2 text-[14px]">
                    ...
                    <span className="text-slate-800 font-medium">
                      {juzMap.get(validJuzs[0])?.join(", ") || "N/A"}
                    </span>
                  </div>
                ) : (
          ...
          <div className="pb-3 border-b border-slate-100 space-y-1">
            ...
            {showMistakeTracker && (
              <div className="flex items-baseline gap-2 text-[14px]">
                ...
                <span className={`font-semibold ${totalMistakes > 0 ? "text-rose-600" : "text-emerald-600"}`}>
                  {totalMistakes}
                </span>
              </div>
            )}
            {showStuckTracker && (
              <div className="flex items-baseline gap-2 text-[14px]">
                ...
                <span className={`font-semibold ${totalStuck > 0 ? "text-amber-600" : "text-emerald-600"}`}>
                  {totalStuck}
                </span>
              </div>
            )}
        ...
        {showMistakeTracker && mistakeMap.size > 0 && (
          <div className="pb-3 border-b border-slate-100">
            <p className="text-[11px] font-bold uppercase tracking-widest text-rose-500 mb-2">
              Mistake Details
            </p>
            <div className="space-y-0.5 text-[13px] text-slate-700">
              {isSingleJuz ? (
                Array.from(mistakeMap.values())[0]?.map((item, idx) => <div key={idx}>• {item}</div>)
              ) : (
                Array.from(mistakeMap.entries()).map(([juzNum, items]) => (
                  <div key={juzNum}>
                    <div className="font-semibold text-slate-500 text-[11px] uppercase tracking-wide mt-1">
                      Juz {juzNum}
                    </div>
                    {items.map((item, idx) => (
        ...
        {showStuckTracker && stuckMap.size > 0 && (
          <div className="pb-3 border-b border-slate-100">
            ...
            <div className="space-y-0.5 text-[13px] text-slate-700">
              {isSingleJuz ? (
                Array.from(stuckMap.values())[0]?.map((item, idx) => <div key={idx}>• {item}</div>)
              ) : (
        ...
        {showCommentSection && comment && comment.trim() !== "" && (
          <div className="pb-3 border-b border-slate-100">
            ...
            <div className="whitespace-pre-wrap text-[13px] text-slate-700 leading-relaxed">
              {comment.trim()}
            </div>
          </div>
        )}
