...
import { findMatchingPeriodSlot } from '@/modules/learning/utils/dailyClassroomUtils';
import type { UseStudentAssessmentOptionsParams, UseStudentAssessmentOptionsReturn } from '../types';
...
export default function useStudentAssessmentOptions({
  ...
  curriculumBookId = '',
  curriculumBookName = '',
}: UseStudentAssessmentOptionsParams): UseStudentAssessmentOptionsReturn {
  // 1. Student Options
  ...
  const bookOptions = useMemo(() => {
    ...
    if (periodSlotId && periodSlotId !== 'ALL') {
      ...
      const targetOrder = matchedSlot?.period_order ?? (matchedSlot as any)?.order ?? (Number(periodSlotId) || null);
