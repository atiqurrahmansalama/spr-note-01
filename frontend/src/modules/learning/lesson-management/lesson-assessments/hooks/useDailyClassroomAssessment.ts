...
import { doesLessonMatchClass, isLessonInSlot, getClassId } from '@/modules/learning/utils/dailyClassroomUtils';
import type {
  UseDailyClassroomAssessmentParams,
  UseDailyClassroomAssessmentReturn,
  AssessmentRowItem,
...
export default function useDailyClassroomAssessment({
  ...
  filteredLessons = [],
  baseFilteredLessons = [],
}: UseDailyClassroomAssessmentParams): UseDailyClassroomAssessmentReturn {
  // ── Assessment Rows (Student × Evaluation join) ─────────────────────────────

  const assessmentRows: AssessmentRowItem[] = useMemo(() => {
    ...
    return (enrolledStudents || []).map((st) => {
      ...
      const stClsName = (st.student_class_name || (st as any).class_name || '').toLowerCase().trim();

      const findClassLesson = (list: any[]) => {
      ...
      return {
        ...
        homework_score:
          matchedEval?.homework_score !== undefined
            ...
            ? matchedEval.score
            : '—',
        total_mistakes: Number(matchedEval?.total_mistakes || 0),
        total_stucks: Number(matchedEval?.total_stucks || 0),
  ...
  const getSlotAssessmentCount = useCallback(
    (slotValue: string | number): number => {
      const targetDate = String(selectedDate || '').split('T')[0];
  ...
  const assessmentMetrics: AssessmentMetricItem[] = useMemo(() => {
