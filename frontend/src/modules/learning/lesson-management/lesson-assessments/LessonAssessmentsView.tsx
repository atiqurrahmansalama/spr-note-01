...
import DailyClassroomFilterControls from '@/modules/learning/components/DailyClassroomFilterControls';
...
import {
  ...
  formatRangeText,
} from '@/modules/learning/utils/dailyClassroomTableHelpers';
import {
  ChecklistIcon,
  EditIcon,
  DeleteIcon,
} from '@/components/ui/Icons';
import { learningStore } from '@/stores';
import { deleteLessonEvaluation as deleteEvaluationAPI } from '@/api/learning';
import { useToast } from '@/context/ToastContext';
import type { LessonAssessmentsViewProps, AssessmentRowItem } from './types';

/**
 * LessonAssessmentsView
 * Main management view for student daily evaluations, mistake/stuck tracking,
 * recitation & homework scores, and teacher remarks.
 */
export default function LessonAssessmentsView({
  filterProps = null,
  assessmentRows = [],
  assessmentMetrics = [],
  ...
  loadData,
  ...restFilterProps
}: LessonAssessmentsViewProps) {
  const { showToast } = useToast();
  const [deletingEvaluation, setDeletingEvaluation] = useState<AssessmentRowItem | null>(null);
  ...
  const handleConfirmDeleteEvaluation = async () => {
    ...
    try {
      if (deletingEvaluation.id) {
        learningStore.deleteEvaluation(tenantId, String(deletingEvaluation.id));
  ...
  // Cards for Assessment Grid View
  const renderAssessmentCard = (row: AssessmentRowItem) => (
    <div
      key={row.id}
      onClick={() => onOpenAssessmentDrawer?.(row.student, row)}
    ...
    >
      <div>
        <div className="flex items-start justify-between gap-2">
          ...
          <span
            className={`text-xs font-semibold px-2 py-0.5 rounded-full border theme-border ${
      ...
      <div className="pt-2 border-t theme-border flex items-center justify-between text-xs theme-text-secondary">
        <span className="truncate max-w-[200px]">
          {row.teacher_remarks !== '—' && row.teacher_remarks ? row.teacher_remarks : 'No remarks yet'}
        </span>
  ...
  return (
    <div className="animate-fade-in space-y-4">
      <UniversalManagementView
        ...
        subtitle="Manage and evaluate daily student performances, mistakes, stucks, and scores"
        icon={ChecklistIcon}
        hideHeader={true}
        isEmbedded={true}
        ...
        onRowClick={(row) => onOpenAssessmentDrawer?.(row.student || row.id, row)}
...
// Named alias exports for legacy compatibility
export { LessonAssessmentsView as StudentAssessmentManagementView };
