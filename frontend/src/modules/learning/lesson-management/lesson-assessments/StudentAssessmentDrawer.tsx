...
import { curriculumStore } from '@/stores';
import { findMatchingPeriodSlot } from '../../utils/dailyClassroomUtils';
...
import type { StudentAssessmentDrawerProps } from './types';
      ...
      <form onSubmit={form.handleSubmit} className="@container p-4 @[480px]:p-6 space-y-6 text-left">
        ...
        {form.hasFixedContext && (
          <ClassroomContextCard
            ...
            departmentName={form.studentDeptObj?.name || form.activeStudent?.department_name || (form.activeStudent as any)?.department || ''}
            className={form.studentClassObj?.name || (form.studentClassObj as any)?.class_name || form.activeStudent?.student_class_name || ''}
        ...
        {(!form.hasFixedContext || form.isEditingContext) && (
          <DrawerSection title="Assessment Context" icon={CalendarIcon}>
              ...
              <div className="grid grid-cols-1 @[480px]:grid-cols-2 gap-3">
                ...
                <CustomSelect
                  ...
                  onChange={(val) => {
                    ...
                    const targetOrder = matchedSlot?.period_order ?? (matchedSlot as any)?.order ?? (Number(val) || null);
                    if (targetOrder !== null && !isNaN(targetOrder)) {
                      const autoBook = form.availableBooks.find((b: any) => {
                ...
                <CustomSelect
                  ...
                  onChange={(val) => {
                    ...
                    const b =
                      ...
                      form.availableBooks.find((item: any) => String(item.id) === String(val));
                    if (b) {
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <CustomInput
                ...
                onChange={(val) => form.setTotalMistakes(val === '' ? 0 : Math.max(0, parseInt(val, 10) || 0))}
                placeholder="0"
              />
              ...
              <CustomInput
                ...
                onChange={(val) => form.setTotalStucks(val === '' ? 0 : Math.max(0, parseInt(val, 10) || 0))}
                placeholder="0"
              />
            </div>
