...
import { DailyLessonItem } from "../types";

export interface UseLessonPlanFormParams {
...
export default function useLessonPlanForm({
  ...
  const [periodSlotId, setPeriodSlotId] = useState<string>(
    lesson?.period_slot ? String(lesson.period_slot) : (defaultPeriodId && defaultPeriodId !== "ALL" ? String(defaultPeriodId) : "")
  );

  const [curriculumBookId, setCurriculumBookId] = useState<string>(lesson?.curriculum_book_id ? String(lesson.curriculum_book_id) : "");
  const [curriculumBookName, setCurriculumBookName] = useState<string>(lesson?.curriculum_book_name || "");
  const [subjectName, setSubjectName] = useState<string>(lesson?.subject_name || "");
  const [teacherName, setTeacherName] = useState<string>(lesson?.teacher_name || "");
  const [lessonTitle, setLessonTitle] = useState<string>(lesson?.lesson_title || "");
  const [startUnit, setStartUnit] = useState<string>(lesson?.start_unit !== undefined && lesson?.start_unit !== "" ? String(lesson.start_unit) : "");
  const [endUnit, setEndUnit] = useState<string>(lesson?.end_unit !== undefined && lesson?.end_unit !== "" ? String(lesson.end_unit) : "");
  const [homeworkTask, setHomeworkTask] = useState<string>(lesson?.homework_task || "");
  const [lessonInstructions, setLessonInstructions] = useState<string>(lesson?.lesson_instructions || "");
  const [assignedScope, setAssignedScope] = useState<string>(() => {
  ...
  const [isCarryForwardOpen, setIsCarryForwardOpen] = useState<boolean>(false);
  ...
  const initialFormSnapshot = useMemo(() => ({
    ...
    periodSlotId: lesson?.period_slot ? String(lesson.period_slot) : (defaultPeriodId && defaultPeriodId !== "ALL" ? String(defaultPeriodId) : ""),
  ...
  const handleBookChange = (val: string) => {
    ...
    const b = availableBooks.find((item: any) => String(item.id) === String(val)) ||
              curriculumBooks.find((item: any) => String(item.id) === String(val));
    if (b) {
  ...
  const handleStartPageChange = (val: any) => {
  ...
  const handleEndPageChange = (val: any) => {
    const rawVal = typeof val === "string" ? val : val?.target?.value || "";
