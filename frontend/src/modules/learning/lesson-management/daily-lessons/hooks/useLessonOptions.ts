...
export interface UseLessonOptionsParams {
  departments?: any[];
  classes?: any[];
  sections?: any[];
  students?: any[];
  ...
  periodSlotId?: string | number;
  curriculumBookId?: string | number;
  teacherName?: string;
}
...
export default function useLessonOptions({
  ...
  const availableBooks = useMemo(
    () => propAvailableBooks || filterCurriculumBooks(curriculumBooks, classId ? String(classId) : undefined, classes, departmentId ? String(departmentId) : undefined),
  ...
  const bookOptions = useMemo(() => {
    const list: Array<{ value: string; label: string }> = [{ value: "", label: "None (Direct Entry)" }];
    ...
    if (periodSlotId && periodSlotId !== "ALL") {
      ...
      const periodMatched: any[] = [];
      const others: any[] = [];

      availableBooks.forEach((b: any) => {
        const isMatch =
  ...
  const teacherOptions = useMemo(() => {
    const list: Array<{ value: string; label: string }> = [{ value: "", label: "None (Unassigned)" }];
