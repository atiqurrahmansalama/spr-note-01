...
import { doesLessonMatchClass, getYesterdayDate } from '../../utils/dailyClassroomUtils';
import { CarryForwardLessonPanelProps, DailyLessonItem } from './types';
...
export default function CarryForwardLessonPanel({
  ...
  tenantId = 'default',
  onApplyLesson,
}: CarryForwardLessonPanelProps) {
  const [carrySourceDate, setCarrySourceDate] = useState<string>(() => getYesterdayDate(currentDate));
  ...
  const availablePreviousDates = useMemo(() => {
    ...
    try {
      ...
      allLessons.forEach((l: DailyLessonItem) => {
  ...
  const sourceLessons: DailyLessonItem[] = useMemo(() => {
    if (!isOpen || !carrySourceDate) return [];
    try {
      const allLessons = learningStore.getDailyLessons(tenantId) || [];
      const targetSource = String(carrySourceDate).split('T')[0];

      return allLessons.filter((l: DailyLessonItem) => {
  ...
  const handleApply = (src: DailyLessonItem) => {
    const sNum = parseInt(String(src.start_unit), 10);
    const eNum = parseInt(String(src.end_unit), 10);
    let nextStart = String(src.start_unit || '');
    let nextEnd = String(src.end_unit || '');
      ...
      <div className="grid grid-cols-1 @[480px]:grid-cols-2 gap-3 items-end">
        ...
        <div className="p-3 rounded-xl border theme-border theme-bg-surface hover:theme-bg-sub/60 transition-colors flex items-center min-h-[46px]">
          <CustomCheckbox
            ...
            onChange={(checked) => setAutoAdvancePages(Boolean(checked))}
