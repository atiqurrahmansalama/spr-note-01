...
export default function StaffLeaveManagementView() {
  ...
  // Action Modals
  const [actioningLeave, setActioningLeave] = useState(null);
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto min-h-screen theme-text-primary animate-fade-in select-none">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          <button
            onClick={loadLeaves}
            className="p-2 rounded-xl theme-bg-surface border theme-border theme-text-secondary hover:theme-text-primary cursor-pointer"
            title="Refresh"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 bg-amber-600 hover:bg-amber-500 text-white rounded-xl text-xs font-semibold shadow transition-all cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredLeaves.map((leave) => {
            ...
            return (
              <div
                key={leave.id}
                className="p-5 rounded-2xl theme-bg-surface border theme-border hover:theme-bg-elevated/40 transition-all flex flex-col justify-between space-y-4"
              >
                <div>
                  {/* Top Badge & Status */}
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] px-2.5 py-0.5 rounded-full font-bold font-mono theme-bg-sub border theme-border theme-text-primary">
                  ...
                  <div className="mt-3">
                    ...
                    <div className="text-xs theme-text-secondary">
                      {leave.staff_designation} • <span className="font-mono">{leave.staff_employee_id}</span>
                    </div>
                  </div>

                  {/* Dates & Duration */}
                  <div className="mt-3 p-3 rounded-xl theme-bg-sub border theme-border space-y-1 text-xs">
                    ...
                    <div className="flex justify-between">
                      <span className="theme-text-secondary">Date Range:</span>
                      <span className="theme-text-primary font-mono">{leave.start_date} → {leave.end_date}</span>
                    </div>
                  </div>

                  {/* Reason */}
                  <div className="mt-3">
                    <span className="text-[11px] font-semibold theme-text-secondary uppercase tracking-wider block mb-1">
                      Reason
                    </span>
                    <p className="text-xs theme-text-primary italic theme-bg-sub p-2.5 rounded-xl border theme-border">
                      "{leave.reason}"
                    </p>
                  </div>
                  ...
                  {/* Admin Remarks */}
                  {leave.admin_remarks && (
                    <div className="mt-2 text-[11px] theme-text-secondary pt-1 border-t theme-border">
                      <span className="font-medium">Remarks by {leave.approved_by_name || 'Admin'}:</span>{' '}
                      <span className="theme-text-primary">{leave.admin_remarks}</span>
                    </div>
                  )}
                </div>
                ...
                {isPending && (
                  <div className="pt-3 border-t theme-border flex items-center justify-end gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-xl bg-rose-600/20 hover:bg-rose-600/30 text-rose-400 border border-rose-500/30 text-xs font-semibold transition-colors cursor-pointer"
                    >
                      Reject
                    </button>
                    <button
                      onClick={() => handleOpenAction(leave, 'APPROVED')}
                      className="px-4 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold shadow transition-all flex items-center gap-1 cursor-pointer"
                    >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md rounded-2xl theme-bg-surface border theme-border p-6 space-y-4 shadow-2xl theme-text-primary">
            <div className="flex items-center justify-between border-b theme-border pb-3">
              ...
              <button onClick={() => setActioningLeave(null)} className="theme-text-secondary hover:theme-text-primary cursor-pointer">
            ...
            <form onSubmit={handleConfirmAction} className="space-y-3">
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-xl text-xs font-semibold text-white transition-all disabled:opacity-50 cursor-pointer ${
                    actioningLeave.status === 'APPROVED'
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md rounded-2xl theme-bg-surface border theme-border p-6 space-y-4 shadow-2xl theme-text-primary">
            <div className="flex items-center justify-between border-b theme-border pb-3">
              ...
              <button onClick={() => setIsApplyOpen(false)} className="theme-text-secondary hover:theme-text-primary cursor-pointer">
                <CloseIcon className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleApplySubmit} className="space-y-3">
              <div>
                ...
                <select
                  ...
                  required
                  className="w-full px-3 py-1.5 theme-bg-sub border theme-border rounded-xl text-xs theme-text-primary cursor-pointer"
                >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-1.5 theme-bg-sub border theme-border rounded-xl text-xs theme-text-primary placeholder:theme-text-secondary resize-none"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-4 py-1.5 bg-amber-600 hover:bg-amber-500 text-white rounded-xl text-xs font-semibold disabled:opacity-50 cursor-pointer"
                >
