...
interface GeneralDutyDrawerFormProps {
...
interface DutyItem {
  ...
  title: string;
  description?: string;
  assigned_day_of_week?: string | null;
  start_time?: string | null;
  end_time?: string | null;
  location?: string;
}

export const GeneralDutyDrawerForm: React.FC<GeneralDutyDrawerFormProps> = ({
  staff,
  onUpdated,
  onCancel,
}) => {
  ...
  const handleDeleteDuty = async (dutyId: string | number) => {
    try {
            ...
            <div className="grid grid-cols-1 @[480px]:grid-cols-2 gap-4">
              <div>
                <CustomTimePicker
                  ...
                  onChange={(val: any) => setForm({ ...form, start_time: val })}
                />
              </div>
              ...
              <div>
                <CustomTimePicker
                  label="End Time"
                  value={form.end_time}
                  onChange={(val: any) => setForm({ ...form, end_time: val })}
                />
              </div>
            </div>
...
export default GeneralDutyDrawerForm;
