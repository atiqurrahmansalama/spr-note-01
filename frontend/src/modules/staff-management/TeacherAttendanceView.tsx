...
export interface TeacherMatrixRow {
  id?: string | number;
...
export interface TeacherMatrixData {
  days_header?: DayHeaderItem[];
  teachers_matrix?: TeacherMatrixRow[];
...
export default function TeacherAttendanceView() {
  ...
  const [matrixData, setMatrixData] = useState<TeacherMatrixData>({
    days_header: [],
    teachers_matrix: [],
  });
  ...
  const enrichedTeacherRows = useMemo(() => {
    ...
    return rawRows.map((row) => {
      const dailyStatuses: Record<string, any> = { ...(row.daily_statuses || {}) };
      let pCount = 0;
      let lCount = 0;
      let aCount = 0;
      let lvCount = 0;
      ...
      days.forEach((d: DayHeaderItem) => {
        ...
        if (isOff) {
          ...
          const rawStatus = dailyStatuses[fullDateStr] || (d.day ? dailyStatuses[d.day] : '') || '';
  ...
  const handleAdminEditCell = (row: any, dateStr: string, currentStatus: string, _slotId?: string) => {
    if (
  ...
  const handleOpenDayAgenda = (dateParam: any) => {
    ...
    const matchedDay = (matrixData?.days_header || enrichedDaysHeader)?.find((d: any) => d.date === fullDate);
    ...
    openRightSidebar({
      ...
      content: (
        <DayAgendaDrawer
          ...
          onClose={closeRightSidebar}
          onOpenEventForm={(eventToEdit: any) => {
            openRightSidebar({
              ...
              content: (
                <TimeScheduleDrawerForm
                  ...
                  onSaveSuccess={() => {
                    closeRightSidebar();
                    setCalendarEventsVersion((v: number) => v + 1);
  ...
  return (
    <PageContainer isFullscreen={isFullscreen} className="space-y-4">
      ...
      <div className="p-4 sm:p-5 rounded-3xl theme-bg-surface border theme-border shadow-xs space-y-4">
        ...
        <div className="border-t theme-border pt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-3.5 items-end">
          ...
          <CustomSelect
            ...
            onChange={(val: string) => setSelectedDeptId(val)}
            options={[
      ...
      >
        <AttendanceTable
          daysHeader={matrixData.days_header && matrixData.days_header.length > 0 ? matrixData.days_header : enrichedDaysHeader}
