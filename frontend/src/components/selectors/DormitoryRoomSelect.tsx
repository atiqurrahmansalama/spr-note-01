...
export interface DormitoryRoomSelectProps {
  value?: string;
  onChange?: (value: string, selectedObj: any) => void;
  rooms?: DormitoryRoom[];
  buildingId?: string;
  branchId?: string;
  roomType?: string;
...
export default function DormitoryRoomSelect({
  ...
  icon = HomeIcon,
  ...rest
}: DormitoryRoomSelectProps) {
  const { activeTenantId } = useTenant();
  const [internalRooms, setInternalRooms] = useState<DormitoryRoom[]>([]);
  ...
  if (buildingId && buildingId !== 'ALL') {
    roomList = roomList.filter((r) => r.building === buildingId || (r as any).building_id === buildingId);
  }
  if (branchId && branchId !== 'ALL') {
    roomList = roomList.filter((r) => r.branch === branchId || (r as any).branch_id === branchId);
  }
  if (roomType && roomType !== 'ALL') {
    roomList = roomList.filter((r) => r.room_type === roomType);
  }

  const options: Array<{ value: string; label: string; raw?: DormitoryRoom }> = [];
  if (allowAll) {
