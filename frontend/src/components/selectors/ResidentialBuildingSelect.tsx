...
export interface ResidentialBuildingSelectProps {
  value?: string;
...
export default function ResidentialBuildingSelect({
  ...
  ...rest
}: ResidentialBuildingSelectProps) {
  const { activeTenantId } = useTenant();
  const [internalBuildings, setInternalBuildings] = useState<ResidentialBuilding[]>([]);
  ...
  if (branchId && branchId !== 'ALL') {
    buildingList = buildingList.filter((b) => b.branch === branchId || (b as any).branch_id === branchId);
  }

  const options: Array<{ value: string; label: string; raw?: ResidentialBuilding }> = [];
  if (allowAll) {
