...
export interface AddressLocationPickerProps {
  value?: string | AddressLocationValue | null;
...
export default function AddressLocationPicker({
  ...
  const addressText = isObject
    ? ((value as AddressLocationValue).address || (value as AddressLocationValue).street_address || '')
    : (typeof value === 'string' ? value : '');
  const latitude = isObject ? (value as AddressLocationValue).latitude ?? null : null;
  const longitude = isObject ? (value as AddressLocationValue).longitude ?? null : null;

  const handleManualTextChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newText = e.target.value;
    if (isObject) {
      onChange?.({
        ...(value as AddressLocationValue),
        address: newText,
  ...
  const handleMapConfirm = (selectedLocation: any) => {
    if (isObject) {
      onChange?.({
        ...(value as AddressLocationValue),
        ...selectedLocation,
  ...
  const handleClearCoords = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (isObject) {
      onChange?.({
        ...(value as AddressLocationValue),
    ...
    <div className={`space-y-1.5 ${className}`}>
      ...
      {isMapOpen && (
        <AddressMapModal
          ...
          initialLocation={isObject ? (value as any) : { address: addressText }}
