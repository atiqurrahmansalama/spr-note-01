...
export function readJSON<T = any>(key: string, defaultValue: T): T {
  try {
...
export function writeJSON<T = any>(key: string, value: T): void {
  try {
...
export const saveStatusStore = {
  get: (): SaveStatusPayload =>
    readJSON(KEYS.SAVE_STATUS, { type: "local", label: "Saved", timestamp: Date.now() }),
  set: (
    statusOrType: string | { type?: string; label?: string; detail?: any },
    ...
    if (typeof statusOrType === "string") {
      ...
      label =
        optionalLabel ||
        (type === "database" ? "Database Synced" : type === "saving" ? "Saving..." : "Saved");
      detail = optionalDetail || null;
    } else if (typeof statusOrType === "object" && statusOrType !== null) {
      type = statusOrType.type || "local";
      label =
        statusOrType.label ||
        (type === "database" ? "Database Synced" : type === "saving" ? "Saving..." : "Saved");
      detail = statusOrType.detail || null;
    }

    const payload: SaveStatusPayload = {
      type,
      label,
      timestamp: Date.now(),
      detail,
...
export const draftReport = {
  getAll: (): any[] => readJSON("spr_draft_reports_list", []),
  getById: (id: string): any | null => {
    const list = readJSON<any[]>("spr_draft_reports_list", []);
    return list.find((d) => d.id === id) || null;
  },
  save: (id: string, formData: any): any | null => {
    if (!id) return null;
    const list = readJSON<any[]>("spr_draft_reports_list", []);
    const payload = {
      id,
...
export const auth = {
  getUser: (): any => readJSON(KEYS.USER, null),
  getUserProfile: (): any => readJSON(KEYS.USER, null),
  saveUser: (u: any): void => writeJSON(KEYS.USER, u),
  saveUserProfile: (u: any): void => writeJSON(KEYS.USER, u),
  getAccessToken: (): string => readString(KEYS.ACCESS_TOKEN),
  getToken: (): string => readString(KEYS.ACCESS_TOKEN),
  saveAccessToken: (t: string): void => writeString(KEYS.ACCESS_TOKEN, t),
  getRefreshToken: (): string => readString(KEYS.REFRESH_TOKEN),
  saveRefreshToken: (t: string): void => writeString(KEYS.REFRESH_TOKEN, t),
...
export const multiAccount = {
  getAccounts: (): any[] => readJSON(KEYS.SAVED_ACCOUNTS, []),

  saveAccount: (accountData: any): any[] => {
  ...
  removeAccount: (identifier: string): any[] => {
    const list = multiAccount.getAccounts();
